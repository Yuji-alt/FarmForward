...
class GrowthController @Inject constructor() {
    ...
    fun setupObserver(lifecycleOwner: LifecycleOwner, viewModel: CropViewModel) {
        viewModel.cropData.observe(lifecycleOwner) { crop ->
            if (crop == null || crop.cropName.isBlank()) {
                ...
                val minText = crop.mindate?.let { "Min Date: ${dateFormat.format(it)}" } ?: "Min Date: N/A"
                val maxText = crop.maxdate?.let { "Max Date: ${dateFormat.format(it)}" } ?: "Max Date: N/A"

                view?.setMinHarvest(minText)
                view?.setMaxHarvest(maxText)
                ...
                view?.setCropImage(getCropImage(crop.cropName))
    ...
    fun onDeleteClicked(viewModel: CropViewModel) {
        ...
        val message = if (today >= minHarvest) {
            "Have you harvested this ${crop.cropName}? Deleting it will remove it permanently."
        } else {
            "This crop is still growing. Are you sure you want to delete it?"
        }

        view?.showDeleteConfirmation(message) {
    ...
    private fun getCropImage(cropName: String): Int {
        ...
        return when {
            lower.contains("corn") -> R.drawable.agricultwo
            lower.contains("rice") -> R.drawable.agricultwo
            else -> R.drawable.agricultwo
        }
    }
